...
// ── Parse Claude's response text into a take object ──────────────────────────
function parseTakeResponse(text) {
  // First try: find the outermost {...} block
  const match = text.match(/\{[\s\S]*\}/);
  if (match) {
    try {
      const parsed = JSON.parse(match[0]);
      if (parsed.take?.text) return parsed.take;
    } catch { /* fall through to lenient path */ }
  }
  // Lenient path: extract "text" value directly in case JSON is malformed
...
async function generateOneTake(client, topic, meta, language) {
  const AI_FALLBACK = [{ name: 'AI Analysis — perspectives generated by Claude', framing: null, url: null }];
  const { prompt, derivedSources } = buildPrompt(topic, meta, language);
  const sources = derivedSources?.length ? derivedSources : AI_FALLBACK;
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      const take    = parseTakeResponse(rawText);
      if (take?.text) return { ...take, position: meta.position, label: meta.label, color: meta.color, sources };
      // bad parse — retry once
    } catch (err) {
      if (attempt === 0) continue; // retry on first failure
    }
  }

  // Both attempts failed — generate a minimal but real-looking fallback via a simpler prompt
  try {
    const fallbackPrompt = `Write 2 sentences giving a ${meta.label} perspective on: "${topic.title}". Be specific and substantive. Reply with only the sentences, no JSON.`;
    const msg = await client.messages.create({
      model:      'claude-haiku-4-5-20251001',
      max_tokens: 200,
      messages:   [{ role: 'user', content: fallbackPrompt }],
    });
    const fallbackText = msg.content[0]?.text?.trim();
    if (fallbackText) {
      return { position: meta.position, label: meta.label, text: fallbackText, color: meta.color, sources };
    }
  } catch { /* give up */ }
