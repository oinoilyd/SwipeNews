...
export function buildPrompt(topic, meta, language = 'English') {
  ...
  // Derive sources from the actual articles — don't rely on Claude to generate them.
  // primaryArts → up to 3 sources shown; each gets name + bias label as framing + url.
  const derivedSources = primaryArts.slice(0, 3).map(a => ({
    name:    a.source,
...
{"take":{"position":${meta.position},"label":"${effectiveLabel}","text":"your 50-80 word take here"}}${language !== 'English' ? `\n\nCRITICAL: Write the entire "text" field in ${language}. Every word of the take must be in ${language} — do not use English.` : ''}`;

  return { prompt, effectiveLabel, singleSource, derivedSources };
}
